...
const userAgents = [
    ...
    const tempFilePath = path.join(process.cwd(), 'scripts', 'cmds', 'cache', `${uuidv4()}.jpg`);
    const headers = {
        'Accept': '*/*',
        'Accept-Encoding': 'gzip, deflate, br, zstd',
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            r(l).on('response', function (response) {
                ...
                p = process.cwd() + '/scripts/cmds/cache' + `/${Date.now()}.${e}`;
...
module.exports.simi = function(type, data) {
  const dataSimPath = path.join(__dirname, 'src', 'sim.json');
  ...
  if (type === 'ask') {
    ...
    else dataSim.push({ id: String(dataSim.length + 1), ask, ans: [ans], by: "", time: new Date().toLocaleTimeString() });
...
module.exports.text2image = async function(text) {
  const headers = {
  ...
  const prompt = text; 
  if (!prompt) {
      return console.log("Please provide a prompt");
  }

  const data = JSON.stringify([prompt, "square", true]);

  axios
      .post("https://ai.codedao.cc/", data, { headers })
      .then((response) => {
          const responseData = response.data;
          
          // Sử dụng regex để tìm imgUrl
