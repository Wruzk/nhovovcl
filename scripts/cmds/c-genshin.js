...
module.exports.onCall = async function ({ api, event, args }) {
  ...
  if (!number) return api.sendMessage("Xem id nhân vật tại đây:https://api.hamanhhung.site/genshin/list", event.threadID);

  try {
    const response = await axios.get(`https://api.hamanhhung.site/genshin/info?id=${number}`);
